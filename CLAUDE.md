...
# S70 — SAME-DAY TICKER SWITCHING IS INSTANT: the /api/report cache is SESSION-STALE (valid
# until the next market close; was TTL 120s) — CRSP → SOFI → back to CRSP serves the morning's
# report with zero regenerate; Run (force) and live still bypass. The frontend report query
# mirrors the boundary (staleTime = msUntilNextClose(), web/src/utils/session.ts), so a
# switch-back doesn't even hit the server. AND the per-run yfinance calls that fired on every
# gather_report are now disk-cached via modules/netcache.py (benefits CLI runs + first views
# too): VIX complex (3 full-history downloads → data/vix_cache/, session-stale, window-
# containment serves as-of sub-windows), SKEW/VVIX (data/tail_cache.json, session-stale),
# fetch_rs/fetch_beta closes (data/rs_cache/ per symbol — SPY dedupes across both + across
# tickers sharing a benchmark), earnings_dates + next_ex_dividend (data/earnings_cache/ +
# data/exdiv_cache/, 24h, empty/null cached too). All never-raise w/ stale fallback. Documented
# tradeoff: --live/Run bypass only the payload cache — the module caches are daily-granularity
# and not force-plumbed (a live run serves the session's cached VIX/SKEW/RS).
...
# S68: LEVEL PROJECTIONS sub-block under the ladder — for the KEY levels (--level, nearest S/R,
# ≤2 confluence zones): travel estimate (~N sessions at recent HV-20 pace) + long-call P&L via
# BS repricing, instant AND paced legs (theta cost of a slow grind). Quoted contracts when the
# --call cache exists (zero network; STALE cache → entry re-modeled at current spot, `entry≈`),
# else a modeled ATM call. IV held constant; caveat unconditional. As-of runs are synthetic-only.
# S69: FOUR tenors — ~45d/~90d/~6mo/~1yr (the long pair is what this project trades; sizing.py
# targets 180–365 DTE). Long-tenor expiries are grid-aware: monthlies preferred, pool widened
# past 150d, and a tenor with nothing inside ±max(30, 0.35×target) is OMITTED not mislabeled
# (SOFI's 1yr = a non-monthly 322d LEAP; CRSP has none). Each expiry also carries a ≤9-strike
# ±25% LADDER off the same chain rows — all repriced server-side, so the web expiry/strike
# pickers only filter (no BS in TS). CLI shows one ATM row per tenor. Net chain calls per --call
# run unchanged (~7, measured): the new tenors consume curve budget. modules/levelproj.py.
# S70: web CUSTOM-PRICE stepper — a number box (± arrows + selectable step) that reprices the
# same contracts at ANY price via GET /api/project/{tkr}?price= (levelproj.project_price →
# project_targets, so one pricing path; no chain fetch, no regenerate). CLI equivalent = --level.
# S71: + a hold-until DATE picker (&date=YYYY-MM-DD) — a third `dated` leg beside instant/paced
# answering "what if it gets there ON that day", i.e. what the wait costs in theta. Past a
# contract's expiry it floors to intrinsic and is marked `exp`. Web-only (calendar dropdown).
...
| `sizing.py` | Live options chain sizing via Tradier API (`MIN_DTE=180`/`MAX_DTE=365` — the tenor window S69's level projections now quote). **ARCHIVED**: lives at `archive/ml_pipeline/sizing.py`, not the repo root | Console only |
...
| `modules/levelproj.py` | **(S68)** Level projections — "what would a move to this KEY level look like": rides INSIDE `ladder["projections"]` (payload contract untouched). Targets = `--level` + nearest S/R + ≤2 nearest confluence zones, deduped ±0.5%. Per target: `travel_sessions` (\|dist\| ÷ HV-20/√252, linear sigma-days capped 252 — first-passage would be slower, pace note says so) + per-contract long-call P&L via BS repricing, TWO legs each: instant (T unchanged) and paced (T − travel×365/252 calendar conversion, floor→intrinsic). **S69 — four tenors + strike ladder**: `callquote.TARGET_DTES` is now `(45, 90, 180, 365)` (the last two are what this project actually TRADES — archive/ml_pipeline/sizing.py targets 180–365 DTE), and every candidate of each expiry's `ladder` is repriced here, so the web selectors filter rows that already exist and NO Black-Scholes is duplicated in TS. Rows carry `kind` (atm/otm/other) + `moneyness`/`delta` for the selector labels; renderers default to the ATM row per tenor. Synthetic legs became `(30, 180, 365)` with a real IV chain — harvested gauge → `curve_iv` (the `--call` IV-by-expiry curve, zero network) → HV-20 proxy, each labeled: the 180d row was previously DEAD, keyed to `atm_iv_180d`, which is empty in every indicators CSV on disk. Quoted path (`reprice_contract`): the `--call` quote this run OR the call3 session cache via `callquote.cached_call_quote` (S68 zero-network sibling of `cached_liquidity`); block dte refreshed from expiry vs (injectable) today; **stale-cache honesty fix** — a stale cache's mids were struck at another spot (probed live: QQQ 692→675 made a +3.6% target read −47%), so under `stale` the ENTRY is re-modeled at CURRENT spot (same strike/IV, ask leg dropped, `entry_modeled` flag, `entry≈` prefix + header note). Synthetic fallback (`synthetic_call`): BS ATM call, "modeled, not a quote". `RISK_FREE=0.04` (gex convention; bs arg order r-BEFORE-T guarded by exact-equality test 66). CLI sub-block under PRICE LADDER (**one ATM row per tenor** — S69 replaced `rows[:4]`, which sliced an expiry-ordered list and would have silently truncated away exactly the new long-dated rows; expiry label is `YY-MM-DD` + DTE since a 2027 LEAP is ambiguous without the year) + one-line at-your-level summary in THESIS CHECK; as-of = synthetic-only (cache read gated; as-of gauges historically valid); web `LadderProjections` in core.tsx with expiry + strike pickers (default: ≥150d tenors × ATM, empty selection disallowed). **S73 pickers are PAGED** — `PICKER_PAGE`=5 pills behind ‹ › arrows (the expiry list is now 12-14 long), window auto-positioned on the first selected option, with an `n–m of N · k selected off-screen` counter so a filtered table is never a mystery. The per-strike multi-select is replaced by the strike-COUNT picker above; `STRIKE_COUNTS` is exactly 5 entries so it never needs its own arrows, options above the deepest available ladder are hidden, and the default is 1 (ATM only — the pre-S73 view). **S70 custom-price stepper**: `project_price(price, spot, …)` builds a one-target ladder and runs it through `project_targets`, so the stepper shares ONE pricing path with the table above it (drift-guarded by a bit-equality test); served by `GET /api/project/{ticker}?price=` off `reportgen.LATEST` + the `--call` session cache (no chain fetch, no regenerate; 409 when nothing has been generated for that ticker yet). Web = number box with ± arrows, a step picker (`STEP_CHOICES`, default ≈0.5% of spot), 250ms debounce and `placeholderData` so the table doesn't blank between steps. Deliberately a round-trip, NOT TS math — duplicating Black-Scholes in the browser is how the stepper would start disagreeing with the block above it. **S71 dated leg**: `_legs` gained `hold_cal`, so every contract can carry a THIRD leg beside instant/paced — `dated`, the value if the price arrives on a date the user picked (calendar days, no trading-day conversion; `project_price(on_date=)` → `?date=YYYY-MM-DD`). instant = T unchanged, paced = T minus the HV-20 travel ESTIMATE, dated = T minus a hold the user CHOSE, so it answers "what does waiting cost me" without a guess. Past a contract's own expiry the leg floors to intrinsic and sets `expired` (rendered `4.25 exp`); dates in the past are refused (422). Web = native `<input type="date">` dropdown calendar + 1w/1m/3m/6m presets + clear, and the two dated columns appear only when a date is set. Pure, offline-testable (tests 66-70 + the S69 trio + S70 + S71) | Imported by `lens.py` (default-on when a ladder builds) |
...
| `modules/netcache.py` | **(S70)** Shared freshness + JSON helpers for the per-run network-call caches: `most_recent_close` (deliberate 10-line DUPLICATE of `pc_oi._most_recent_close` — keeps pc_oi/tradier out of features.py's import chain; drift-guarded by test 71), `session_fresh` (valid until the next weekday 16:00 ET), `fresh_hours` (wall-clock TTL), `load_json`/`save_json` (never raise). Powers: `features._cached_close_frame` (VIX complex → `data/vix_cache/`, session-stale, window-CONTAINMENT check serves as-of sub-windows and the min-start/max-end rule means windows only widen), `sentiment._fetch_tail` (SKEW/VVIX → `data/tail_cache.json`), `setupcheck._cached_closes` (per-symbol 6mo closes → `data/rs_cache/` — `fetch_rs` + `fetch_beta` share it, so SPY is fetched once per session across both and across tickers), `features.earnings_dates`/`next_ex_dividend` (24h result caches → `data/earnings_cache/` + `data/exdiv_cache/`, empty list / null cached too, ex-div `days` recomputed on hit + past-date treated stale), and the API's session-stale `report_cache` (`api/cache.py get_report`/`put_report`). Every cache: stale fallback on fetch failure, never raises | Imported by `modules/features.py`, `modules/sentiment.py`, `modules/setupcheck.py`, `api/cache.py` |
...
| `modules/setupcheck.py` | **(S41)** Default-on SETUP CHECK — transparent ✓/✗/– completeness checklist over reads the lens already computes: HTF alignment, momentum room, volume confirmation, **relative strength vs sector benchmark** (`fetch_rs` — `TICKER_BENCHMARK` first entry / SPY fallback, one yfinance fetch, series aligned to common last date, best-effort n/a; **S67 `extra=[(sym,label)]` rider** — comparators like the equal-weight twin join the SAME single download as a symbol list, results in `out["extra"]`, `extra=None` return shape byte-identical and row 4 untouched per the S65 frozen-rows rule), value-area location, vol regime (very-rich IV ✗; VIX stress = S21 contrarian note, not a fail), catalyst timing (earnings ≤7d / Tier-1 macro ≤1d = flagged `–`, NOT failed). Explicitly a blind-spot catcher, not a probability. **S70**: `fetch_rs`/`fetch_beta` closes come from the shared session-stale `_cached_closes` (`data/rs_cache/`, per symbol) — SPY downloads once per session across both fns and across tickers; `fetch_beta` gained a `data_dir` kwarg | Imported by `lens.py` |
...
| `modules/breadth.py` | **(S45, +S67 MARKET BREADTH section)** Equal-weight market breadth — RSP−SPY + QQQE−QQQ 20d/63d relative returns (cap-weighted "SPY up" can be a narrow mega-cap-led tape; the equal-weight twin reads the AVERAGE stock). Pure `read_breadth` (tags broad-led/narrow/mixed on a ±0.5% band; percentile off the rolling 20d-spread series — short horizons + own-series percentile sidestep the secular mega-cap drift; **S67** adds per-pair `spark` ≤60-pt spread history, `cap_off_high` distance from the 252d closing high, and `div_state`/`div_desc` from pure `divergence_read` — two-sided: NARROWING = cap leg ≤2% off its high while the EW spread is negative or ≤25th percentile / BROAD = highs confirmed / REPAIR = >5% off high with EW leading) + `fetch_breadth` (ONE batched yfinance download — S67 adds the `PARTICIPATION` IWM−SPY small-cap pair, 5 symbols total, deliberately NOT in `PAIRS` so the backdrop segment is byte-unchanged; cache `{"pairs","participation"}` w/ TTL-fresh shape gate, `data/breadth_cache.json` ~6h, stale fallback serves the OLD shape → consumers `.get()`, never raises). Shown on the lens MARKET BACKDROP line + the S67 default-on **MARKET BREADTH section** (CLI table + divergence lines + the `ticker_ew` "vs the average stock" line; payload key `breadth`; as-of omitted; web `SecBreadth` w/ PctBar + sparklines). CONTEXT only — never a model feature, deliberately NOT a risk-scorecard factor (S43 lesson) | Imported by `lens.py` + `market_context.py` |
| `modules/callquote.py` | **(S46)** Long-call viability quote for `lens.py --call` — the directional instrument's carry math off the live Tradier chain: nearest expiry to each of `TARGET_DTES` (`_select_expiries`), ATM + ~0.375Δ candidates (`pick_call_candidates`; no-bid strikes skipped) with mid premium, breakeven move, **theta/day as % of premium**, OI/spread (+at-ask line when mid understates >3%), earnings- and ex-div-aware per-expiry notes, an ATM-IV-by-expiry curve (`curve_read`, ≤`CURVE_MAX_EXPIRIES`=7 monthlies ≤`CURVE_MAX_DTE`=400) and a chain `liquidity_grade` (ATM-region median spread + OI → tight/ok/wide/dead). **S72/S73 — EVERY MONTHLY** (replaced S69's nearest-to-4-target-DTEs rule, which quoted only 4 expiries and silently hid the rest — SOFI lists monthlies at 113/141/232 DTE that never became pills). `select_expiries` takes every monthly with `EXPIRY_MIN_DTE`(0) ≤ dte ≤ `EXPIRY_MAX_DTE`(1100), capped at `MAX_EXPIRIES`(20) — bounds are guard rails, not curation. Measured live: **12 SOFI / 14 QQQ / 13 AMD / 5 CRSP**, out to ~869 DTE. No tenor is hidden and none needs a "~1yr" label (the label was the only thing that could lie). `monthly_expiries` is **list-aware and holiday-aware**: per (year, month) it takes the third Friday if listed, else the day before — because when the third Friday is a market holiday the monthly shifts to Thursday and `pc_oi.is_monthly_expiry` (date-only: Friday, day 15-21) calls it non-monthly. Probed live 2026-07-30: SOFI/QQQ/AMD all list `2027-06-17` (Thu) and NOT `2027-06-18`, since Juneteenth 2027 is a Saturday observed that Friday — a naive monthlies-only filter would have DROPPED that LEAP. `TARGET_DTES` survives as the **CLI curation** set (`nearest_to_targets`): a terminal has no picker and 12 expiries × 5 targets is 60 rows, so the CLI prints the 4 closest to 45/90/180/365d and says how many were quoted. The IV curve needs no second fetch pass any more (every window monthly is already quoted) — it went from ≤7 points to all of them, free. `strike_ladder` returns the `LADDER_MAX`=**25** tradeable calls NEAREST SPOT within ±`LADDER_PCT`=**40%**, off the SAME parsed rows (zero extra network), tagged atm/otm/other + `moneyness`, riding each block as `ladder`. **"All strikes" was measured and rejected**: QQQ lists 2,661 tradeable calls across its monthlies (149 inside ±25% alone, strikes 205-1115 vs a 684 spot) → ~13k repriced rows, ~3 MB payload, a 200-pill picker. An even-sampled variant spanning the full ±band was also tried and reverted — it put ~$20 between adjacent QQQ rungs, so "3 strikes around spot" returned spot ±$20 instead of the three adjacent strikes. **The count is the user's**: the web has a `STRIKE_COUNTS` (1/3/5/9/25) picker meaning "N nearest spot **per expiry**" (a union-wide N collapsed to 2 rows on some expiries and 4 on others). Reach is the expiry picker's axis; this one is proximity. Chain calls per run = exactly `len(select_expiries(...))` — measured 12 SOFI (~9s) / 14 QQQ (~16s, its chains are ~250 rows each). Report payload 366KB SOFI / 783KB QQQ (the DOM stays small — only SELECTED rows render). Cached session-stale like volquote (SCOPEKEY **call5**, bumped for the S73 ladder; `force=` under `--live`); `cached_liquidity` serves the grade zero-network to the default-on OPTIONS line | Imported by `lens.py` (`--call` + the default-on liquidity line) |
