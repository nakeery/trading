...
# 25 tests, ~2-3s. Requires data/QQQ_indicators.csv + data/QQQ_backtest_results.csv.
```
...
| lens.py | 1 (ticker; or `--ticker QQQ JPM …` to skip prompt) + argparse flags (--thesis / --level / --geo / --no-intraday / --no-vix / --no-color / --candle box\|braille\|sixel / --candle-px N / --prev N / --no-refresh / --refresh / --pc-oi [all\|near\|leaps\|monthly …] / --vol / --live / --squeeze / --insider) |
...
| `tests/test_smoke.py` | 25 pytest regression guards (signal hierarchy, STRONG ENTRY baseline, vol thresholds, signal logic, S16 threshold-sensitivity, econ_calendar loads, Days_to_* bounds, days-to-specific-event, regime thresholds/gate/NaN, next_event/upcoming shape, sentiment labelers, S35 geocontext stress/composite, S38 volquote snap + expiry selection, S39 vol-history study, S40 vol_setup factors + live-bar append + intraday top-up merge, S41 shortint parsers/scorecard + setupcheck + fng parser, S42 insider Form 4 parse + cluster detection) | Run manually; requires `data/QQQ_*.csv` |
