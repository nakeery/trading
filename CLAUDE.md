...
| `modules/buzz.py` | **(S56)** ApeWisdom retail-attention gauge riding the `--squeeze` block — keyless API, top ~400 tickers cached ~6h (`data/buzz_cache.json`, ONE feed fetch serves every ticker); `buzz_read` (pure) → rank/mentions/24h-change; unranked → silently absent. Attention ≠ direction — caveat printed | Imported by `lens.py` (`--squeeze`) |
