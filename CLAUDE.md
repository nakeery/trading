...
# BS repricing, instant AND paced legs (theta cost of a slow grind). Quoted 45/90-DTE contracts
# when the --call cache exists (zero network; STALE cache → entry re-modeled at current spot,
# `entry≈`), else a modeled ATM call from harvested IV (30d + 180d LEAPS tenor). IV held
# constant; caveat unconditional. As-of runs are synthetic-only. modules/levelproj.py.
...
# 111 tests (S68; 3 skipped), ~3-12s. Requires data/QQQ_indicators.csv + data/QQQ_backtest_results.csv.
```
...
| `modules/levelproj.py` | **(S68)** Level projections — "what would a move to this KEY level look like": rides INSIDE `ladder["projections"]` (payload contract untouched). Targets = `--level` + nearest S/R + ≤2 nearest confluence zones, deduped ±0.5%. Per target: `travel_sessions` (\|dist\| ÷ HV-20/√252, linear sigma-days capped 252 — first-passage would be slower, pace note says so) + per-contract long-call P&L via BS repricing, TWO legs each: instant (T unchanged) and paced (T − travel×365/252 calendar conversion, floor→intrinsic). Quoted path (`reprice_contract`): the `--call` quote this run OR the call2 session cache via `callquote.cached_call_quote` (S68 zero-network sibling of `cached_liquidity`); block dte refreshed from expiry vs (injectable) today; **stale-cache honesty fix** — a stale cache's mids were struck at another spot (probed live: QQQ 692→675 made a +3.6% target read −47%), so under `stale` the ENTRY is re-modeled at CURRENT spot (same strike/IV, ask leg dropped, `entry_modeled` flag, `entry≈` prefix + header note). Synthetic fallback (`synthetic_call`): BS ATM call at 30d (harvested ATM IV, else HV-20 proxy — labeled) + 180d (LEAPS tenor when the 180d gauge exists), "modeled, not a quote". `RISK_FREE=0.04` (gex convention; bs arg order r-BEFORE-T guarded by exact-equality test 66). CLI sub-block under PRICE LADDER (non-user targets ATM-only, rows[:4], caveat unconditional: IV constant/no skew/straight-line pace/not advice) + one-line at-your-level summary in THESIS CHECK; as-of = synthetic-only (cache read gated; as-of gauges historically valid); web `LadderProjections` in core.tsx. Pure, offline-testable (tests 66-70) | Imported by `lens.py` (default-on when a ladder builds) |
