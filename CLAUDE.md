...
# 36 tests, ~2-7s. Requires data/QQQ_indicators.csv + data/QQQ_backtest_results.csv.
```
...
| lens.py | 1 (ticker; or `--ticker QQQ JPM …` to skip prompt) + argparse flags (--thesis / --level / --geo / --no-intraday / --no-vix / --no-color / --candle box\|braille\|sixel\|none / --candle-px N / --prev N / --no-refresh / --refresh / --pc-oi [all\|near\|leaps\|monthly …] / --vol / --call / --live / --squeeze / --insider) |
| lens_web.py | 0 prompts — browser UI; run `.\trade\Scripts\python.exe -m streamlit run lens_web.py` (not for piped/Claude runs; use lens.py) |
