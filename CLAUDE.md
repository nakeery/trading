...
# 120 tests (S70; 3 skipped), ~3-12s. Requires data/QQQ_indicators.csv + data/QQQ_backtest_results.csv.
```
...
| `modules/callquote.py` | **(S46)** Long-call viability quote for `lens.py --call` — the directional instrument's carry math off the live Tradier chain: nearest expiry to each of `TARGET_DTES` (`_select_expiries`), ATM + ~0.375Δ candidates (`pick_call_candidates`; no-bid strikes skipped) with mid premium, breakeven move, **theta/day as % of premium**, OI/spread (+at-ask line when mid understates >3%), earnings- and ex-div-aware per-expiry notes, an ATM-IV-by-expiry curve (`curve_read`, ≤`CURVE_MAX_EXPIRIES`=7 monthlies ≤`CURVE_MAX_DTE`=400) and a chain `liquidity_grade` (ATM-region median spread + OI → tight/ok/wide/dead). **S69**: `TARGET_DTES = (45, 90, 180, 365)` — the long pair is the tenor this project trades. Long-tenor selection is grid-aware because the LEAPS ladder is sparse and ticker-specific: monthlies (3rd Friday = liquidity) win while one sits inside `tenor_tol` (±max(30, 0.35×target)), past `LONG_DTE_MIN`=150 the pool widens to EVERY listed expiry (SOFI's 1yr is a non-monthly 322d LEAP — the nearest monthly, 232d, is 133 days off), and a target with nothing inside tolerance is **omitted** rather than mislabeled (CRSP: 171d then 542d → no 1yr row). New `strike_ladder` returns ≤`LADDER_MAX`=9 tradeable calls within ±`LADDER_PCT`=25% of spot off the SAME parsed rows (zero extra network), tagged atm/otm/other + `moneyness`, riding each block as `ladder` for the level-projection strike selector. Net chain calls per run are UNCHANGED (~7, measured): the extra quote tenors seed `curve_pts` and so consume the curve's expiry budget instead of adding to it. Cached session-stale like volquote (SCOPEKEY **call3**, bumped for the ladder shape; `force=` under `--live`); `cached_liquidity` serves the grade zero-network to the default-on OPTIONS line | Imported by `lens.py` (`--call` + the default-on liquidity line) |
